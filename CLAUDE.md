...
- `sbin/updatewebui [options] [branch]` — fetches a branch zip from GitHub and installs `www/*` → `/var/www`, `sbin/*` → `/usr/sbin`, `bin/*` → `/usr/bin` (the same payload `tools/build-dist.sh` hands buildroot). This is the canonical "deploy from source" path; default branch is `master`. `--help` lists the options, `--dry-run` reports what would change, and `--restore` takes the installed copies away again so the firmware's own WebUI shows through — and puts back what the install replaced, so the camera lands where it was rather than merely stripped.
    ...
    **The installer is not on the camera (v10).** `/usr/sbin/updatewebui` is `sbin/updatewebui-fetch`, a stub that downloads `sbin/updatewebui` from GitHub — from the branch being installed, where one was named — checks that what came back is that script rather than an error page, caches it at `/etc/webui/updatewebui.sh` and `exec`s it with the arguments it was given. Two reasons. The installer is 48 KB of a rootfs that is a fixed-size squashfs partition, and an 8 MB gk7202v300 had 12.0 KB of headroom in its 5120 KB before the week's WebUI work took it 12 KB over, so the board stopped building entirely. And a script that installs itself is always one release behind: the copy running an install was the one the *previous* install left, so a fix to the installer took effect on the run after the one that fetched it — backwards for the run bringing the fix down. That used to be answered by `hand_over`, which compared `scr_version` against the downloaded tree's and `exec`ed it mid-flight behind a one-hop `UPDATEWEBUI_HANDOVER` guard; fetching first makes the running copy the newest by construction and all of it is gone, along with `--no-self-update` and the version bump that a change to the variant skip list used to oblige. Nothing is given up in the ordinary case, because this script's whole job is to download a tree from GitHub: a camera that cannot reach GitHub could not use it either way. The exception is `--restore`, which reaches no network and says so in its own help — so a cached copy is used as-is there rather than refreshed, and any cache is by definition one that ran an install on this camera. The download path is not new; `/etc/profile` has shipped a `majestic_webui()` function fetching this same file for years, without `-f` (so a 404 was written to the file and fed to `sh`) and without passing arguments on. `tests/updatewebui-fetch.test.js` pins the branch parsing, both refusals, the offline fallback and the `--restore` promise, each shown able to fail.
