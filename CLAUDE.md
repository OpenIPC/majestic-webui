...
- **A comment explains the code, not the review that produced it.** We have git for that: the branch, the review thread, the bot or agent that asked. None of it is in a checkout, so a `(Qodo #342)` or `(PR #342)` tag sends the next reader somewhere they cannot go, and means nothing at all once the branch is deleted. Cite the **issue** that reported the fault (`(#273)`, `OpenIPC/majestic#311`) — a document that stays readable and says what the code cannot — or cite nothing and let the sentence stand on its own. Never write a comment addressed to a reviewer.
