...
5. **Dirty tracking.** After rendering, `state.initial[dot] = field.getValue()` snapshots each control. On every `input`/`change`, `updateDirty()` recomputes which fields differ, toggles a `.mj-dirty` class on the row (left border highlight from `bootstrap.override.css`), and hands the count to `renderToolbar()` — which shows the bar and the Save button, or removes both when nothing is pending. The count line also says what Save will cost, *before* it is pressed — "Save restarts the video streams." or "Save keeps the streams running." — from `changeCost()` over the pending set. It is said there and not per row because on a HiSilicon build 156 of 202 keys are pipeline-class: a mark on each would be a mark on nearly every row (#316).
