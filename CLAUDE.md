...
**Placing writes on drop, not on Save** — the one deliberate exception on this page, because a position you have not sent is one you cannot see. `applyNow()` POSTs the `osd` subtree and then re-baselines `state.initial` for those keys, so the save bar does not offer a Save that would do nothing. Everything else on the leaf stages normally. Dragging snaps to the picture's edges and middles (`SNAP` 12px): a **named anchor** survives a change of resolution where a raw pixel offset does not, and the readout says which one it picked. A centred axis writes offset 0 because majestic's `place_anchored` ignores the offset there — the schema's own `visibleWhen` hides that field to match.
