...
- **The IR-cut machinery is three modules and a ladder of evidence**, because a misconfigured filter is invisible everywhere else: the camera streams, records, answers ONVIF and reports healthy counters while sending a magenta picture, and the commonest fault is not a wrong value but a *missing* one — with no `nightMode.irCutPin1` majestic never drives the filter at all.
    ...
    - **The one thing an owner may assert is that there is no filter at all**, and `ircut-check.js`'s `claim()` is the only place that decides whether the assertion still holds. Nothing measurable separates an unwired filter from an absent one, so the Dashboard's × records the answer on the camera (`/etc/webui/ircut.conf`, via `j/ircut.cgi` — a claim majestic neither stores nor serves, which is why this one is still a CGI). Wiring a filter contradicts it, so `claim()` drops it. **Which page asks matters more than what it asks**: the drop used to run only on the Dashboard, and the Dashboard is the one page on which nobody wires anything — wire the filter on Day / Night, take it away again, and no Dashboard was open while a pad existed, so the claim outlived the wiring that contradicted it and went on hiding a warning about a filter the camera could no longer move (#367). Nor is it recoverable after the fact: clearing those fields *removes* the keys rather than zeroing them, so the config ends byte-identical to how it started. Day / Night therefore asks after every save, while the pad is still there. The general form — an invalidation rule has to be applied where the invalidating event happens, not only in the state it leaves behind — is worth carrying to any other dismissal that outlives a page load.
