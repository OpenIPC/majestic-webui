...
**The Live adjustments leaf is the whole `image` section, with the picture (#316).** Three rules, in `www/a/mj-tree.js` and pinned by `tests/tree.test.js`. The first group with any `x-live` key *owns* the leaf, and those keys — from every section of that group — are **lifted** onto it beside the picture; that set is the only thing the flag decides. A section the leaf lifts *more* of than it leaves is **absorbed**: its leftovers render on the leaf too, in a plain card under the deck, and it has no page of its own — `image`, six of eight keys live, is that section. A section the leaf lifts *less* of keeps its page and says where the rest went (`liftedNote`, destination first), so thirty exposure controls do not move house because one went live. The old rule — skip every x-live key, the Live leaf draws them — cut orientation in two: mirror and flip on the picture as the Orientation pad, the quarter turns (`image.rotate`, a VPSS operation that swaps the stream's width and height, hence a pipeline reload rather than a live knob) on a two-row Image page whose first line listed the six settings it did not have, each page pointing at the other. Now the Orientation column carries the pad *and* a **Quarter turn** row (None / 90° / 270°) over a hidden `rotate` field on the pin-map pattern, its group note saying only *on Save* — the save bar that appears on the press says the rest, and a sentence under the row explaining why the picture does not move was a second explanation of one fact; `?tab=image` lands on the leaf. **Stock** in the tone strip resets the four integer knobs only: orientation is a mounting decision, not a tone, and a button that says four must not turn the picture over.
