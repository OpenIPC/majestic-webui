...
- **One round-trip per save** — the camera walks the whole tree, then reloads the SDK and writes the file back exactly once. The legacy per-key `yaml-cli` loop is gone.
