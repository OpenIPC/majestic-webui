...
<!-- No page heading: the nav underlines "Live", and a heading here would be a
     ...
     to sit here -- lives in camera.cgi, whose Live section pairs the same
     preview with the real-time adjustments. The one exception is the PTZ pad,
     which steers rather than configures; it stays on the video until the
     multi-user split decides who may steer.
