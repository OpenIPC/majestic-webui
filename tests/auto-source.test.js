...
(async () => {
	...
	group('the player container is watched, not only the window');
	{
		...
		await tick();
		// The stage, not the video nodes, and not #mj-player: the viewport
		// clamp resizes the stage on height-only viewport changes the parent
		// never sees, so the stage is the element whose size actually tracks
		// the picture.
